...
class BiLSTM(nn.Module):
	...
	def __init__(self, config, vocab_size, word_embeddings):
		...
		self.net = nn.Sequential(nn.Linear(3, self.config.ll_hidden_size), nn.ReLU(), nn.Dropout(p=self.dropout), nn.Linear(self.config.ll_hidden_size,self.config.output_size), nn.Softmax())
	...
	def forward(self, pairs):
		...
		h_n1 = torch.cat([h_n1[0,:,:], h_n1[1,:,:]],-1)
		h_n2 = torch.cat([h_n2[0,:,:], h_n2[1,:,:]],-1)

		print("Shape of hidden state is {} after concat".format(h_n1.shape))
		
		h_n1 = h_n1.view(self.config.batch_size,2*self.config.hidden_size)
		h_n2 = h_n2.view(self.config.batch_size,2*self.config.hidden_size)
